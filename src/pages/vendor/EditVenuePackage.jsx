...
import { ChevronRight, ChevronLeft, Check, Upload, DollarSign, Image, HelpCircle, Video, Eye, EyeOff, Package, Info, MapPin, GalleryHorizontal, ListVideo, BadgeCheck, Loader2, Trash2 } from 'lucide-react';
...
const EditVenuePackage = () => {
    ...
    useEffect(() => {
        ...
        const fetchPackageData = async (categories) => {
            try {
                setLoading(true);
                const token = localStorage.getItem("vendorToken");
                const res = await api.get(`/vendor/venue-packages/${packageId}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                ...
                const categoryOption = categories.find(c => c.value === pkg.venueCategory?._id) || null;

                setFormData({
                    title: pkg.title,
                    description: pkg.description,
                    startingPrice: pkg.startingPrice,
                    venueCategory: categoryOption,
                ...
                if (categoryOption) {
                    setServices(categoryOption.services);
                    const initialServiceData = {};
                    if(pkg.services){
                        categoryOption.services.forEach(service => {
        ...
        fetchVenueCategories().then(categories => {
            if (packageId && categories.length > 0) {
                fetchPackageData(categories);
            }
        });
                ...
                if (json.results[0]) {
                    ...
                    if (matchedState) {
                        ...
                        if (matchedCity) {
                            ...
                            setSelectedCity(null); // Reset city if not found
                        }
                    } else {
    ...
    const handleDeleteAlbum = async (albumIndex) => {
        ...
        try {
          const token = localStorage.getItem("vendorToken");
          await api.delete(`/vendor/venue-packages/${packageId}/albums/${album._id}`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
          const res = await api.get(`/vendor/venue-packages/${packageId}`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const handleDeletePhoto = async (albumIndex, photoId) => {
        ...
        try {
            ...
            await api.delete(`/vendor/venue-packages/${packageId}/albums/${albumId}/photos/${photoId}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
    ...
    const nextStep = async () => {
        ...
        if (currentStep === 1) { // Update Package with Location
            ...
            try {
                const token = localStorage.getItem("vendorToken");
                await api.put(`/vendor/venue-packages/${packageId}/basic`, data, {
        ...
        } else if (currentStep === 2) { // Upload/Update Albums
            setLoading(true);
            const newAlbums = formData.albums.filter(a => a.newPhotos && a.newPhotos.length > 0);
            
            if (newAlbums.length === 0) {
                toast.info('No new albums to upload, skipping.');
                setCurrentStep(3);
                setLoading(false);
                return;
            }
        ...
        } else if (currentStep === 3) { // Update Videos
             setLoading(true);
            try {
                const token = localStorage.getItem("vendorToken");
                // In a real PUT/PATCH, you'd send only the changes. Here we replace all.
                // A more optimized API would accept patches.
                await api.put(`/vendor/venue-packages/${packageId}/videos`, { videos: formData.videos.filter(v => v.title && v.url) }, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                setCurrentStep(4);
        ...
        } else if (currentStep === 4) { // Update FAQs
            setLoading(true);
            try {
                const token = localStorage.getItem("vendorToken");
                await api.put(`/vendor/venue-packages/${packageId}/faqs`, { faqs: formData.faqs.filter(f => f.question && f.answer) }, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                setCurrentStep(5);
        ...
        return (
            <div className="min-h-[70vh] bg-gray-50 flex items-center justify-center p-6 font-poppins">
                <div className="bg-white rounded-2xl shadow-xl p-12 text-center max-w-md w-full">
                    ...
                    <p className="text-gray-600 mt-3">Your venue package has been successfully updated.</p>
    ...
    const selectStyles = {
        control: (provided) => ({ ...provided, borderColor: '#d1d5db', '&:hover': { borderColor: '#ef4444' }, boxShadow: 'none', borderRadius: '0.5rem', padding: '0.15rem 0' }),
        option: (provided, state) => ({ ...provided, backgroundColor: state.isSelected ? '#ef4444' : state.isFocused ? '#fee2e2' : 'white', color: state.isSelected ? 'white' : 'black' }),
    };
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-10 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="mt-1 text-sm text-gray-600">Update your package details, albums, and more.</p>
                </div>
                           ...
                           <div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <Select options={venueCategoryOptions} isDisabled={true} value={formData.venueCategory} onChange={handleCategoryChange} styles={selectStyles} placeholder="Select a category..." />
                                </div>
                            </div>
                            <div className="pt-4 mt-4 border-t">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <label htmlFor="featured-image-upload" className="cursor-pointer bg-white border border-gray-300 text-gray-700 font-semibold py-2 px-4 rounded-lg hover:bg-gray-50 transition-all">
                                        ...
                                        {featuredImage ? 'Change Image' : 'Upload New Image'}
                                    </label>
                                </div>
                            </div>
                            <div className="pt-4 mt-4 border-t">
                                   ...
                                   <div className="flex justify-between items-center">
                                    ...
                                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                                        {album.images && album.images.map((photo) => (
                                            <div key={photo._id} className="relative aspect-square group">
                                                <img src={photo.url} alt="photo" className="w-full h-full object-cover rounded-md"/>
                                                <button 
                                                    onClick={() => handleDeletePhoto(i, photo._id)}
                    ...
                    {currentStep === 5 && (
                         <div className="text-center py-8">
                            ...
                            <p className="text-sm text-gray-600 mb-6 max-w-md mx-auto">
                                'Public' packages are visible to all clients. 'Private' packages are hidden.
                            </p>
