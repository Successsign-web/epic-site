...
const fetchServiceCategories = async () => {
    try {
        ...
        const res = await api.get('/vendor/service-categories', {
            headers: { Authorization: `Bearer ${token}` },
        });

        console.log("Service category API response:", res.data);
        ...
        const options = res.data.data.map(cat => ({
            ...
            subCategories: cat.subCategories ? cat.subCategories.map(sub => ({
                value: sub._id,
                label: sub.name,
                originalObject: sub // Store the full sub-category object
            })) : [],
        }));
...
useEffect(() => {
        ...
        navigator.geolocation.getCurrentPosition(async (pos) => {
            ...
            try {
                ...
                if (json.results[0]) {
                    ...
                    const addr = json.results[0].formatted_formatted_address;
    ...
    const handleServiceCategoryChange = async (option) => {
    setFormData(p => ({ ...p, serviceCategory: option, subServiceCategory: null }));
    setServices([]);
    setServiceData({});
    setSubServiceCategoryOptions([]);

    if (!option) return;

    try {
        const token = localStorage.getItem("vendorToken");
        const res = await api.get(`/vendor/service-categories/${option.value}/service-sub-categories`, {
            headers: { Authorization: `Bearer ${token}` }
        });
        ...
        setSubServiceCategoryOptions(formatted);
   ...
   const handleSubServiceCategoryChange = (option) => {
    setFormData(p => ({ ...p, subServiceCategory: option }));
    ...
    const nextStep = async () => {
        ...
        if (currentStep === 1) { // Create Package with Location
            if (!formData.title || !formData.startingPrice || !formData.serviceCategory || !formData.subServiceCategory || !formData.fullAddress || !formData.city || !formData.state) {
            ...
            data.append('serviceCategory', formData.serviceCategory.value);
            data.append('serviceSubCategory', formData.subServiceCategory.value);
        ...
        if (Object.keys(servicesObject).length > 0) {
            ...
            try {
                const token = localStorage.getItem("vendorToken");
                const res = await api.post('/vendor/service-packages', data, {
                    headers: { 'Authorization': `Bearer ${token}`,
            ...
            try {
                ...
                for (const video of validVideos) {
                    ...
                    await api.post(`/vendor/service-packages/${packageId}/videos`, data, {
                        headers: { 
            ...
            try {
                ...
                for (const faq of validFaqs) {
                    ...
                    await api.post(`/vendor/service-packages/${packageId}/faqs`, data, {
                        headers: { 
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/x-www-form-urlencoded'
                        }
        ...
        return (
            <div className="min-h-[70vh] bg-gray-50 flex items-center justify-center p-6 font-poppins">
                <div className="bg-white rounded-2xl shadow-xl p-12 text-center max-w-md w-full">
                    ...
                    <p className="text-gray-600 mt-3">Your new service package has been successfully created and is ready.</p>
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-10 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-bold text-gray-900">Add New Service Package</h1>
    ...
    />
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <Select options={serviceCategoryOptions} value={formData.serviceCategory} onChange={handleServiceCategoryChange} styles={selectStyles} placeholder="Select a category..." />
                                </div>
                                {formData.serviceCategory && (
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-700 mb-1.5">Sub-Service Category *</label>
                                        <Select 
                                            options={subServiceCategoryOptions} 
                                            value={formData.subServiceCategory} 
                                            onChange={handleSubServiceCategoryChange} 
                                            styles={selectStyles} 
                                            placeholder="Select a sub-category..." 
                                            isDisabled={!formData.serviceCategory}
                                        />
                                    </div>
                                )}
                            </div>
                ...
                <div className="pt-4 mt-4 border-t">
                    ...
                    {!formData.serviceCategory ? (
                        <p className="text-sm text-gray-500">Please select a service category to see available services.</p>
                    ) : loading ? (
