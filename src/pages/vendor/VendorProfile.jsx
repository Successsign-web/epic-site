      ...
      <div className="max-w-6xl mx-auto px-4">
              ...
              <div className="ml-8 flex-1">
                {isEditing ? (
                  <input
                    ...
                    className="text-4xl font-bold border-b-4 border-red-600 outline-none w-full mb-2"
                  />
                ) : (
                  <h2 className="text-4xl font-bold text-gray-800">{vendor.vendorName}</h2>
                )}
                ...
                {isEditing ? (
                  <input
                    ...
                    onChange={handleInputChange}
                    className="text-lg text-gray-600 border-b-2 border-gray-300 outline-none"
                    placeholder="Contact Person"
                  />
                ) : (
                  <p className="text-lg text-gray-600">{vendor.contactPerson} • Wedding Venue</p>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl shadow text-center">
            ...
            <p className="text-2xl font-bold">₹{vendor.walletBalance.toLocaleString()}</p>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white p-8 rounded-2xl shadow-lg">
            <h3 className="font-bold text-xl mb-6 flex items-center gap-3">
              <Users className="w-6 h-6 text-red-600" /> Business Details
            </h3>
            <div className="space-y-5">
              <div>
                ...
                {isEditing ? (
                  <input type="number" name="workingSince" value={formData.workingSince} onChange={handleInputChange}
                    className="w-full border-b-2 border-gray-300 focus:border-red-600 outline-none text-lg font-medium" />
                ) : (
                  <p className="text-lg font-medium">{vendor.workingSince}</p>
                )}
              </div>
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="text-lg font-medium">{vendor.experience} Years</p>
                )}
              </div>
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="text-lg font-medium">{vendor.teamSize} Members</p>
                )}
              </div>
            </div>
          </div>
              ...
              <div>
                ...
                {isEditing ? (
                  <input type="text" name="phone" value={formData.phone} onChange={handleInputChange}
                    className="w-full border-b-2 border-gray-300 focus:border-red-600 outline-none text-lg" placeholder="9876543210" />
                ) : (
                  <p className="text-lg font-medium flex items-center gap-2"><Phone className="w-5 h-5" /> +91 {vendor.phone}</p>
                )}
              </div>
              ...
              <div>
                ...
                {isEditing ? (
                  <input type="email" name="email" value={formData.email} onChange={handleInputChange}
                    className="w-full border-b-2 border-gray-300 focus:border-red-600 outline-none text-lg" />
                ) : (
                  <p className="text-lg font-medium flex items-center gap-2"><Mail className="w-5 h-5" /> {vendor.email}</p>
                )}
              </div>
              ...
              <div>
                ...
                {isEditing ? (
                  <textarea name="address" value={formData.address} onChange={handleInputChange} rows={2}
                    className="w-full border rounded-lg px-3 py-2 focus:border-red-600 outline-none" />
                ) : (
                  <p className="text-lg font-medium flex items-start gap-2"><MapPin className="w-5 h-5 mt-1" /> {vendor.locality}, {vendor.city}</p>
                )}
              </div>
              ...
              <div>
                ...
                {isEditing ? (
                  <input type="url" name="website" value={formData.website} onChange={handleInputChange}
                    className="w-full border-b-2 border-gray-300 focus:border-red-600 outline-none text-lg" placeholder="https://yourwebsite.com" />
                ) : vendor.website ? (
                  <a href={vendor.website} target="_blank" rel="noopener noreferrer" className="text-red-600 hover:underline flex items-center gap-2">
         ...
         <div className="bg-white p-8 rounded-2xl shadow-lg">
            <h3 className="text-2xl font-bold mb-6 flex items-center gap-3"><FileText className="w-7 h-7 text-red-600" /> Documents</h3>
